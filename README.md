...
- 🎡 **Колесо удачи** — `/wheel`: бесплатная крутка в день, скидки 10–30% на подписку
  с таймером 24ч, бонусы LLM-действий и джекпот-день
- 💳 **Подписка / Telegram Payments** — тарифы в `SUBSCRIPTION_PLANS`, оплата в самом
  Telegram; если провайдер не настроен в `.env` — покупка покажет заглушку
...
- `SUBSCRIPTION_PLANS=1:99,7:499,30:1299` — тарифы `дней:цена в копейках` через запятую
- `PAYMENTS_PROVIDER_TOKEN=` — токен платежного провайдера из @BotFather (раздел Payments). Пустое значение = покупка показывает заглушку.
- `PAYMENTS_CURRENCY=RUB` — валюта платежей
