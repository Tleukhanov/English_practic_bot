...
- 💳 **Подписка через Kaspi QR** — `/premium`: тарифы в `SUBSCRIPTION_PLANS` (тенге),
  после оплаты жмёшь «Я оплатил», присылаешь чек — админ подтверждает, подписка активна
...
- `PROMO_UNLIMITED_CODE=<свой_код>` — промокод безлимита: пользователь шлёт боту `/promo <код>`, лимит для него снимается навсегда. Пустое значение — промокоды выключены.
...
**Подписка через Kaspi QR (оплата вручную):**
- `SUBSCRIPTION_PLANS=1:990,7:1990,30:1990` — тарифы `дней:цена в тенге` через запятую
- `ADMIN_TG_ID=` — твой Telegram ID: сюда бот присылает чеки с кнопками подтвердить/отклонить
- `KASPI_QR_PATH=` — путь к картинке со статичным Kaspi QR, см. [Проверка оплаты](#-проверка-оплаты)

> **Как проходит оплата:** юзер выбирает тариф в `/premium`, получает QR, сумму и
> комментарий `BOT-XXXX`. Переводит в Kaspi, жмёт «Я оплатил» и присылает чек (фото).
> Бот пересылает чек тебе (`ADMIN_TG_ID`), ты жмёшь «✅ Подтвердить» — подписка
> активируется мгновенно (картинка QR кладётся по `KASPI_QR_PATH`).

## 📱 Telegram Mini App

Mini App открывает интерактивный урок в полноэкранном интерфейсе Telegram. Архитектура и
этапы разработки описаны в [MINI_APP.md](MINI_APP.md).

### Настройка

```env
WEBAPP_URL=https://<туннель>.trycloudflare.com
WEBAPP_PORT=8081
WEBAPP_HOST=127.0.0.1
```

`WEBAPP_URL` должен вести на публичный HTTPS-адрес, `WEBAPP_PORT` — совпадать с портом
API, а `WEBAPP_HOST` — задавать адрес локального сервера. После настройки команда `/lesson`
показывает кнопку «📱 Урок в Mini App».

> Имя переменной может уточниться при интеграции.

### Локальный запуск

1. В первом терминале запусти `python -m bot.main`: бот и API поднимутся на `WEBAPP_PORT`.
2. Во втором терминале из корня проекта запусти `.\scripts\tunnel.ps1`.
3. Скопируй напечатанный URL в `.env` как `WEBAPP_URL=https://<туннель>.trycloudflare.com`
   и перезапусти бота.
4. Открой Telegram, выполни `/lesson` и нажми «📱 Урок в Mini App».

Telegram не открывает Mini App через `localhost`: нужен публичный HTTPS. Cloudflared Quick
Tunnel бесплатен, но после перезапуска выдаёт новый URL — тогда обнови `WEBAPP_URL` и снова
перезапусти бота.

Для постоянного домена запусти `.\scripts\deploy_miniapp.ps1`. Скрипт только проверяет
локальный порт и печатает инструкции по nginx и certbot, не выполняя деплой.

### Безопасность

Бэкенд валидирует Telegram `initData` для API-запросов. `WEBAPP_INSECURE_AUTH=1` допустим
только для локальной отладки; на публичном сервере не используй его.
...
> Бот работает под непривилегированным пользователем (uid 10001). Перед первым
> запуском дай хосту писать в каталог данных:
>
> ```bash
